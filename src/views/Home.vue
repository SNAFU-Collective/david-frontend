<template>
  <div>
    ...
    <v-container style="z-index: 2">
      ...
      <!--Manualy added this div here to onboard users into minting nfts on ethereum-->
            ...
            >
              <v-list-item three-line>
                <v-list-item-content>
                  ...
                  <v-list-item-subtitle>There are still {{ sale.info.maxSupply - sale.info.totalSupply }} NFTs left to
                    be minted on {{ sale.network.name }}
              ...
              <v-card-actions>
                <v-btn outlined
                       rounded
                       class="mt-3"
                       :disabled="!sale.saleAvailable" :to="`/sale/${sale.chainId}`">
                  {{ sale.saleAvailable ? `Mint now` : 'Soldout' }}
                </v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
